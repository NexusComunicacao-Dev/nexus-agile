...
// GET (ajustado para params Promise)
...
export async function PATCH(
  req: Request,
  context: { params: Promise<{ sessionId: string }> } // UPDATED
) {
  const { sessionId } = await context.params; // NEW
  const { userId, error } = await requireUser();
  if (error) return error;
  const body = await req.json().catch(() => ({}));
  const { pokerSessions, pokerVotes, projects, stories } = await collections();
  const session = await pokerSessions.findOne({ _id: sessionId }); // UPDATED
  if (!session) return NextResponse.json({ error: "Not found" }, { status: 404 });
  const proj = await projects.findOne({ _id: session.projectId, memberIds: userId! });
  if (!proj) return NextResponse.json({ error: "Forbidden" }, { status: 403 });
  ...
  if (body.action === "vote") {
    ...
    if (!(session.deck as any[]).includes(value) && value !== null)
  ...
  if (body.action === "reveal") {
    ...
    const consensus = numeric.length
      ? Math.round(numeric.reduce((a, b) => a + b, 0) / numeric.length)
      : undefined;
    await pokerSessions.updateOne(
      { _id: session._id },
      { $set: { status: "revealed", revealedAt: new Date().toISOString(), consensusPoints: consensus } }
    );
